...
// Simple test endpoint that doesn't require OpenAI
...
app.post('/api/assistant', async (req, res) => {
  console.log('Received assistant API request');
  try {
    ...
    if (!message) {
      console.log('Missing message in assistant request');
    ...
    console.log('Calling OpenAI for assistant response');
    ...
    console.log('OpenAI API response received for assistant request');
    
    // Get the assistant's response
    const assistantResponse = response.choices[0].message.content;
    console.log('Assistant response content:', assistantResponse.substring(0, 50) + '...');
