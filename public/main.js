...
function init() {
    ...
    console.log('Assistant UI elements:', {
        assistantInput: assistantInput ? 'found' : 'missing',
        assistantSendButton: assistantSendButton ? 'found' : 'missing',
        assistantMessagesElement: assistantMessagesElement ? 'found' : 'missing',
        assistantTyping: assistantTyping ? 'found' : 'missing'
    });
...
function setupEventListeners() {
    // Journal entry modal close button
    const journalModal = document.getElementById('journal-modal');
    const closeJournalModalBtn = journalModal.querySelector('.close-modal');
    if (closeJournalModalBtn) {
        closeJournalModalBtn.addEventListener('click', () => {
            console.log("Journal modal close button clicked");
    ...
    // All entries modal close button
    const allEntriesModal = document.getElementById('all-entries-modal');
    const closeAllEntriesBtn = allEntriesModal.querySelector('.close-all-entries');
    if (closeAllEntriesBtn) {
        closeAllEntriesBtn.addEventListener('click', () => {
            console.log("All entries modal close button clicked");
...
async function sendAssistantMessage() {
    ...
    console.log("Assistant processing state:", assistantProcessing);
    
    // Don't send empty messages
    if (!message || assistantProcessing) {
        console.log("Message empty or assistant is already processing, returning");
        return;
    }
    ...
    if (inJournalingMode) {
        console.log("In journaling mode, checking if this is a response to journal preview");
        // First check if this is a response to journal preview
        if (journalEntryInProgress && processJournalPreviewResponse(message)) {
            console.log("Processed as journal preview response");
            return;
        }
        
        console.log("Checking if this is a journal answer");
        // Otherwise, process as an answer to a journal question
        if (processJournalAnswer(message)) {
            console.log("Processed as journal answer");
            return;
        }
    }
    
    console.log("Processing as regular message to assistant API");
    ...
    try {
        ...
        console.log("Calling assistant API endpoint with data:", {
            message: message,
            currentEmotion: currentEmotion,
            emotionHistoryLength: recentHistory.length
        });
        ...
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                console.log("Received assistant response:", data.response);
            ...
            // FALLBACK: Show a hardcoded response if API call fails
            console.log("Using fallback response due to API error");
            
            // Add fallback assistant message
            addAssistantMessage({
                sender: 'ai',
                message: `I noticed you're feeling ${currentEmotion}. I'd like to respond to "${message}" but I'm having some trouble connecting right now. Could you try again in a moment?`,
                emotion: currentEmotion,
                timestamp: new Date().toISOString()
            });
            ...
            assistantMessages.push({
                role: 'ai',
                message: "Fallback response due to connection error",
                emotion: currentEmotion,
                timestamp: new Date().toISOString()
            });
            
            showGuidance('Connection issue with assistant. Using fallback response.', 'warning');
        }
    } catch (error) {
        ...
        addAssistantMessage({
            sender: 'ai',
            message: "I'm having trouble responding right now. There might be a connection issue.",
            emotion: currentEmotion,
            timestamp: new Date().toISOString()
        });
    } finally {
        ...
        console.log("Assistant processing complete, ready for next message");
    }
}
...
function processJournalPreviewResponse(message) {
    console.log("Processing journal preview response:", message);
    console.log("Journal entry in progress:", journalEntryInProgress ? "exists" : "null");
    
    if (!journalEntryInProgress) {
        console.log("No journal entry in progress, returning false");
        return false;
    }
    ...
    if (response === 'save') {
        ...
        return true;
    } else if (response === 'edit' || response.includes('edit')) {
...
function addAssistantMessage(messageData) {
    console.log("Adding assistant message:", {
        ...
        message: messageData.message ? messageData.message.substring(0, 30) + '...' : 'empty',
        emotion: messageData.emotion,
        isJournalPrompt: messageData.isJournalPrompt,
        isJournalPreview: messageData.isJournalPreview
    });
    
    // Check if element exists
    if (!assistantMessagesElement) {
        console.error("assistantMessagesElement not found!");
        return;
    }
    ...
    console.log("Appending message element to chat container");
...
function showEditableJournalEntry(entry) {
    console.log("Showing editable journal entry:", entry);
    const modal = document.getElementById('journal-modal');
    const dateElement = document.getElementById('entry-date');
    const emotionsElement = document.getElementById('entry-emotions');
    const contentElement = document.getElementById('entry-content');
    ...
    // First try the visible class (from the CSS)
    modal.classList.add('visible');
    
    // Also try the show class (from the original code)
    modal.classList.add('show');
...
async function testApiConnection() {
    ...
    showGuidance("Testing API connection...", "info");
    ...
    try {
        ...
        if (response.ok) {
            ...
            addAssistantMessage({
                sender: 'ai',
                message: `API test successful! Server responded with: "${data.message}"`,
                emotion: currentEmotion,
                timestamp: new Date().toISOString()
            });
            
            showGuidance("API connection successful!", "success");
        } else {
            console.error("Test API error response:", await response.text());
            showGuidance("API test failed. Check console for details.", "error");
            
            // Add error message to chat
            addAssistantMessage({
                sender: 'ai',
                message: "API test failed. There might be a connection issue with the server.",
                emotion: currentEmotion,
                timestamp: new Date().toISOString()
            });
        }
        ...
        showGuidance("API connection error: " + error.message, "error");
